...
export default async function BlogPage({
  ...
  function getCategoryStyle(tagName: string) {
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '0 24px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, padding: '16px 0', borderBottom: '1px solid #DDD8CC', flexWrap: 'wrap' }}>
          ...
          <Link href="/blog" style={{
            ...
            border: `2px solid ${!tag ? '#1C1A16' : '#5A5855'}`,
            backgroundColor: !tag ? '#1C1A16' : '#F5F0E8',
            color: !tag ? '#F5F0E8' : '#1C1A16',
            textDecoration: 'none',
          }}>
            Tất Cả
          </Link>
          ...
          {categories?.map(cat => {
            ...
            return (
              <Link key={cat.id} href={`/blog?tag=${encodeURIComponent(cat.name)}`} style={{
                display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: '0.5px',
                textTransform: 'uppercase', padding: '8px 18px', borderRadius: 2,
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: '0 20px', padding: '24px 0', borderBottom: '1px solid #DDD8CC', alignItems: 'start' }}>
              ...
              <div>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 6 }}>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, padding: '3px 8px', borderRadius: 2, textTransform: 'uppercase', letterSpacing: '0.5px', ...getCategoryStyle(post.tag) }}>
                    {post.tag}
                  </span>
        ...
        {totalPages > 1 && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '32px 0', flexWrap: 'wrap' }}>
            {currentPage > 1 && (
              <Link href={`/blog?${tag ? `tag=${encodeURIComponent(tag)}&` : ''}page=${currentPage - 1}`}
                style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-mono)', fontSize: 12, border: '2px solid #5A5855', backgroundColor: '#F5F0E8', color: '#1C1A16', padding: '8px 14px', borderRadius: 2, textDecoration: 'none', minWidth: 38 }}>←</Link>
            )}
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
              <Link key={p} href={`/blog?${tag ? `tag=${encodeURIComponent(tag)}&` : ''}page=${p}`}
                style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-mono)', fontSize: 12, border: `2px solid ${p === currentPage ? '#1C1A16' : '#5A5855'}`, backgroundColor: p === currentPage ? '#1C1A16' : '#F5F0E8', color: p === currentPage ? '#F5F0E8' : '#1C1A16', padding: '8px 14px', borderRadius: 2, textDecoration: 'none', minWidth: 38 }}>{p}</Link>
            ))}
