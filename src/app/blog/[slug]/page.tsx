...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [{ data: categories }, { data: reactions }, { data: comments }, { data: related }] = await Promise.all([
    ...
    supabase.from('posts').select('id, title, slug, excerpt, tag, created_at, thumbnail_url').eq('published', true).eq('tag', post.tag).neq('id', post.id).limit(3),
  ])
    ...
    <div style={{ background: '#F5F0E8', minHeight: '100vh' }}>
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '0 24px' }}>
        ...
        <div style={{ maxWidth: 660, padding: '40px 0' }}>
          <a href="/blog" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: '1px', textTransform: 'uppercase', backgroundColor: 'transparent', color: '#1C1A16', border: '2px solid #1C1A16', padding: '9px 18px', borderRadius: 2, marginBottom: 32, textDecoration: 'none' }}>← Quay lại Blog</a>

          <div style={{ marginBottom: 14 }}>
            <Link href={`/blog?tag=${encodeURIComponent(post.tag)}`} style={{ fontFamily: 'var(--font-mono)', fontSize: 10, padding: '3px 8px', borderRadius: 2, textTransform: 'uppercase', letterSpacing: '0.5px', background: tagBg, color: tagColor, textDecoration: 'none' }}>{post.tag}</Link>
          </div>

          <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(26px, 4vw, 38px)', fontWeight: 400, lineHeight: 1.2, letterSpacing: '-0.8px', marginBottom: 20, color: '#1C1A16' }}>{post.title}</h1>
          ...
          <div style={{ display: 'flex', gap: 14, padding: '14px 0', borderTop: '1px solid #DDD8CC', borderBottom: '1px solid #DDD8CC', marginBottom: 36, flexWrap: 'wrap' }}>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#9A9895' }}>{new Date(post.created_at).toLocaleDateString('vi-VN', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
          ...
          <div className="article-content" style={{ fontSize: 16, lineHeight: 1.85, color: '#5A5855', fontWeight: 300 }} dangerouslySetInnerHTML={{ __html: post.content || '' }} />
          ...
          {related && related.length > 0 && (
            <div style={{ marginTop: 48, paddingTop: 32, borderTop: '1px solid #DDD8CC' }}>
              <div style={{ fontFamily: 'var(--font-serif)', fontSize: 13, fontWeight: 400, letterSpacing: '2px', textTransform: 'uppercase', color: '#9A9895', marginBottom: 20 }}>Bài viết liên quan</div>
              {related.map((p: any) => {
                  ...
                  <Link key={p.id} href={`/blog/${p.slug}`} style={{ textDecoration: 'none' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr', gap: '0 14px', padding: '16px 0', borderBottom: '1px solid #DDD8CC', alignItems: 'start' }}>
                      <div style={{ width: 80, height: 60, borderRadius: 2, overflow: 'hidden', background: '#2C3E50', flexShrink: 0 }}>
                        {p.thumbnail_url && <img src={p.thumbnail_url} alt={p.title} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
                      </div>
                      <div>
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, padding: '2px 6px', borderRadius: 2, textTransform: 'uppercase', letterSpacing: '0.5px', background: relCat?.color_bg || '#EAF0EA', color: relCat?.color_text || '#3D5A3E' }}>{p.tag}</span>
                          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#9A9895' }}>{new Date(p.created_at).toLocaleDateString('vi-VN')}</span>
                        </div>
                        <div style={{ fontFamily: 'var(--font-serif)', fontSize: 16, fontWeight: 400, lineHeight: 1.3, color: '#1C1A16' }}>{p.title}</div>
                      </div>
                    </div>
                  </Link>
                )
