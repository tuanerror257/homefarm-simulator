...
export default async function BlogPostPage({
  ...
  const { data: reactions } = await supabase
    .from('reactions').select('type').eq('post_id', post.id)
  ...
  const { data: comments } = await supabase
    .from('comments').select('*').eq('post_id', post.id)
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '0 24px' }}>
        ...
        <div style={{ maxWidth: 660, padding: '40px 0' }}>
          ...
          <div style={{ marginBottom: 14 }}>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, padding: '3px 8px', borderRadius: 2, textTransform: 'uppercase', letterSpacing: '0.5px', ...tagStyle((post as Post).tag) }}>
              {(post as Post).tag}
            </span>
          </div>
