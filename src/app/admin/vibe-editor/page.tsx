...
export default function VibeEditorPage() {
  ...
  useEffect(() => {
    ...
    async function loadVibe() {
      ...
      const { data } = await supabase.from('vibes').select('*').eq('id', editId).single()
      if (data) {
        setText(data.text)
  ...
  async function handleSave() {
    ...
    if (editId) {
      await supabase.from('vibes').update({ text: text.trim(), published, updated_at: new Date().toISOString() }).eq('id', editId)
    } else {
      await supabase.from('vibes').insert({ text: text.trim(), published, created_at: new Date().toISOString() })
    }
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '0 24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '24px 0 20px', borderBottom: '1px solid #DDD8CC' }}>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            {saveMsg && (
              <span style={{ fontFamily: 'var(--font-sans)', fontSize: 13, color: '#3D5A3E' }}>{saveMsg}</span>
            )}
            <button
              onClick={handleSave}
              disabled={saving}
        ...
        <div style={{ padding: '40px 0' }}>
          ...
          <div style={{ marginBottom: 8 }}>
            <div style={{ fontFamily: 'var(--font-serif)', fontSize: 13, fontWeight: 400, letterSpacing: '2px', textTransform: 'uppercase', color: '#9A9895', marginBottom: 4 }}>
              Góc Vô Tri
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 16 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#9A9895', letterSpacing: '1px', textTransform: 'uppercase' }}>
                Hiển thị công khai
              </span>
              <div
