      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 24px' }}>
        ...
        <div style={{ padding: '36px 0' }}>
          ...
          {!pending?.length ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 1, background: '#DDD8CC', border: '1px solid #DDD8CC', marginBottom: 40 }}>
              {(pending as any[]).map((comment) => (
                <div key={comment.id} style={{ background: '#FAF7F2', padding: '20px 24px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                    <div>
                      <span style={{ fontSize: 13, fontWeight: 500, color: '#1C1A16', fontFamily: 'var(--font-sans)' }}>{comment.author_name}</span>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#9A9895', marginLeft: 10 }}>{comment.author_email}</span>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#9A9895', marginLeft: 10 }}>{new Date(comment.created_at).toLocaleDateString('vi-VN')}</span>
                    </div>
                  ...
                  <div style={{ fontSize: 14, color: '#5A5855', lineHeight: 1.65, fontFamily: 'var(--font-sans)', marginBottom: 8 }}>{comment.body}</div>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: '#9A9895' }}>
                    Bài: <a href={`/blog/${(comment.posts as any)?.slug}`} target="_blank" style={{ color: '#3D5A3E', textDecoration: 'none' }}>{(comment.posts as any)?.title}</a>
                  </div>
                </div>
              ))}
            </div>
          ...
          {!approved?.length ? (
            <div style={{ fontFamily: 'var(--font-sans)', fontSize: 13, color: '#9A9895', padding: '20px 0' }}>Chưa có comment nào được duyệt</div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {(approved as any[]).map((comment) => (
                  <tr key={comment.id}>
                    <td style={{ padding: '12px 0', borderBottom: '1px solid #DDD8CC', fontSize: 13, color: '#1C1A16', whiteSpace: 'nowrap', paddingRight: 16 }}>{comment.author_name}</td>
                    <td style={{ padding: '12px 0', borderBottom: '1px solid #DDD8CC', fontSize: 13, color: '#5A5855', maxWidth: 300 }}>
                      <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 300 }}>{comment.body}</div>
                    </td>
                    <td style={{ padding: '12px 0', borderBottom: '1px solid #DDD8CC', whiteSpace: 'nowrap', paddingRight: 16 }}>
                    ...
                    <td style={{ padding: '12px 0', borderBottom: '1px solid #DDD8CC', fontFamily: 'var(--font-mono)', fontSize: 10, color: '#9A9895', whiteSpace: 'nowrap', paddingRight: 16 }}>{new Date(comment.created_at).toLocaleDateString('vi-VN')}</td>
